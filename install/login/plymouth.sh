...
if [ "$(plymouth-set-default-theme)" != "mateos" ]; then
  sudo cp -r "$HOME/.local/share/mateos/default/plymouth" /usr/share/plymouth/themes/mateos/
  sudo plymouth-set-default-theme mateos
fi
...
if [ ! -f /etc/systemd/system/mateos-seamless-login.service ]; then
  cat <<EOF | sudo tee /etc/systemd/system/mateos-seamless-login.service
[Unit]
Description=MateOS Seamless Auto-Login
Documentation=https://github.com/basecamp/mateos
...
# Enable mateos-seamless-login.service only if not already enabled
if ! systemctl is-enabled mateos-seamless-login.service | grep -q enabled; then
  sudo systemctl enable mateos-seamless-login.service
fi
