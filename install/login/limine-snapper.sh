if command -v limine &>/dev/null; then
  sudo tee /etc/mkinitcpio.conf.d/mateos_hooks.conf <<EOF >/dev/null
...
#timeout: 3
default_entry: 2
interface_branding: MateOS Bootloader
...
EOF
  ...
  if [[ -z ${MATEOS_CHROOT_INSTALL:-} ]]; then
...
if [[ -n $EFI ]] && efibootmgr &>/dev/null && ! efibootmgr | grep -q MateOS &&
