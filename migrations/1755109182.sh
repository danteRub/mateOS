...
# Users can still opt-in to Cloudflare DNS using: mateos-setup-dns cloudflare
...
if [ -f /etc/systemd/resolved.conf ]; then
  ...
  if [ -f /etc/systemd/network/99-mateos-dns.network ]; then
    sudo rm -f /etc/systemd/network/99-mateos-dns.network
  ...
  echo "To use Cloudflare DNS, run: mateos-setup-dns Cloudflare"
fi