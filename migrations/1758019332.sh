...
mateos-refresh-config uwsm/default
mateos-refresh-config uwsm/env
mateos-state set relaunch-required
...
if grep -q "bindd = SUPER, N, Neovim" ~/.config/hypr/bindings.conf; then
  sed -i '/SUPER, N, Neovim, exec/ c\bindd = SUPER, N, Editor, exec, mateos-launch-editor' ~/.config/hypr/bindings.conf
fi
